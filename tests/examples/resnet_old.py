...
from model import resnet_164, conv33fc
import argparse
...
parser = argparse.ArgumentParser(
                    ...
                    description='Trains a ResNet164-Bottleneck/3conv3fc on CIFAR10',
...
val_loader = DataLoader(CIFAR10(root=CIFAR10_DIR, train=False, transform=
                                ...
                                num_workers=WORKERS, pin_memory=True)
# get resnet-164
def get_model(output_classes: int, seed: int, model_name: str='resnet'):
    if model_name == "resnet":
        model = resnet_164(output_classes, seed)
    else:
        model = conv33fc(output_classes, seed)
    if USE_CUDA:
        model = model.cuda()
    return model
...
if __name__ == '__main__':
    model = get_model(10, int(args.seed), "resnet")
    # ckpt = torch.load('epoch-150-model_1.pt')
    # model.load_state_dict(ckpt['state_dict'])
    train(model, int(args.id))
    # validate(model, nn.CrossEntropyLoss())
