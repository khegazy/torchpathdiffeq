...
class ParallelAdaptiveStepsizeSolver(SolverBase):
    ...
    def _get_initial_t_steps(self, t, t_init, t_final, inforce_endpoints=False):
        ...
        if inforce_endpoints:
            if t_init != t[0,0]:
                ...
                inp = torch.tensor([
                    t_init.unsqueeze(0), t[0,-1].unsqueeze(0)],
                    device=self.device
                )
                if t.shape[-1] == 1:
                    inp = inp.unsqueeze(-1)
            ...
            if t_final != t[-1,-1]:
                ...
                inp = torch.tensor(
                    [t[-1,0].unsqueeze(0), t_final.unsqueeze(0)],
                    device=self.device
                )
                if t.shape[-1] == 1:
                    inp = inp.unsqueeze(-1)
    ...
    def _interweave_evaluations(
        ...
        # TODO: uncomment when fixing numerical issue
    ...
    def integrate(
        ...
        if t is not None:
            ...
            if len(t.shape) == 3:
                ...
                #TODO: Uncomment once fixing numerical issue
                #assert torch.all(
            ...
            while error_ratios is None or torch.any(error_ratios > 1.):
                ...
                if y is not None and max_batch is not None:
                    ...
                    if n_next_steps > max_steps:
                        ...
                        eval_mask = torch.cumsum(eval_counts, dim=0).to(torch.int).to(self.device)
                        eval_mask = eval_mask <= max_steps
                        y = y[eval_mask]
                        t = t[eval_mask]
                ...
                if torch.all(error_ratios <= 1.):
                    ...
                    if max_batch is not None:
                        if loss.requires_grad:
                            #TODO: Gradient currently does allreduce
                            loss.backward()
                        del y
                        y = None
                    #print(t[:,:,0])
...
class ParallelUniformAdaptiveStepsizeSolver(ParallelAdaptiveStepsizeSolver):
    ...
    def _t_step_interpolate(self, t_left, t_right):
        ...
        steps = dt*self.method.tableau.c.unsqueeze(-1)
    ...
    def _remove_excess_t(self, t, remove_idxs):
        ...
        replace_idxs = remove_idxs-torch.arange(
            len(remove_idxs), device=self.device
        )
        t_pruned[replace_idxs] = t_replace
        #print(t_replace[:5,:,0])
        #print(t_pruned[:5,:,0])
