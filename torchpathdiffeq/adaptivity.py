...
class IntegralAdaptivity(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def _add_initial_y(self, ode_fxn, t, t_init=0., t_final=1.):
        if t is not None and len(t.shape) == 1:
            t = t.unsqueeze(-1)
        #t_steps = t_spacing_fxn(t[:-1], t[1:])
        t_steps = self._initial_t_steps(t).to(torch.float64)
    ...
    def adaptively_add_y(
            self, ode_fxn, y, t, error_ratios, t_init, t_final
        ):
        """
        t : [t-1, p1]
        y : [t-1, p1]
        ...
        if y is None:
            return self._add_initial_y(ode_fxn=ode_fxn, t=t, t_init=t_init, t_final=t_final)
    ...
    def __adaptively_add_y(self, ode_fxn, y, t, error_ratios=None, t_add=None, idxs_add=None):
        """
        t : [t]
        y : [t-1, p1]
        t_add : [ta] time between t[idx-1] and t[idx]
    ...
    def remove_excess_y(self, t, error_ratios_2steps, remove_fxn):
