...
class RKParallelAdaptiveStepsizeSolver(ParallelAdaptiveStepsizeSolver):
    ...
    def _calculate_integral(self, t, y, y0=0, degr=degree.P1):
        ...
        tableau_b = self._calculate_tableau_b(dt, h, degr=degr)
        ...
        RK_steps = h*torch.sum(tableau_b*y_steps, dim=-1).unsqueeze(1)   # Sum over k evaluations weighted by c
        if verbose:
    ...
    def _calculate_tableau_b(self, dt, h, degr):
        if degr == degree.P:
            tableau_b_fxn = self.tableau_b_p
        elif degr == degree.P1:
            tableau_b_fxn = self.tableau_b_p1
        
        b = tableau_b_fxn(dt, h, degr)
